...
def chunk_dplr_delta_rule_ref(
    ...
    q, k, v, a, b, gk = map(
        lambda x: F.pad(x, (0, 0, 0, pad_len)).to(torch.float32), [q, k, v, a, b, gk]
    )
    B, H, L, DK = q.shape
    DV = v.shape[-1]
    q = q * scale
...
def test_chunk(
    ...
    q, k, v, a, b, gk, h0 = map(
        lambda x: x.to(device).requires_grad_(True), (q, k, v, a, b, gk, h0)
    )
    ...
    ref_dq, ref_dk, ref_dv, ref_da, ref_db, ref_dg, ref_dh0 = (
        q.grad,
        k.grad,
        v.grad,
        a.grad,
    ...
    tri_dq, tri_dk, tri_dv, tri_da, tri_db, tri_dg, tri_dh0 = (
        q.grad,
        k.grad,
        v.grad,
        a.grad,
    ...
    if (
        gate_logit_normalizer >= 1 and ref_dg.norm() > 0.01
