...
def rwkv_seq_mix_kernel(
    x,              # 输入序列张量(batch, seq_len, hidden_dim)
    x_prev,         # 前一个时间步的状态(batch, seq_len, hidden_dim)
    mix_k,          # 混合系数K
    output,         # 输出张量
    ...
    # 加载当前值
    curr_x = tl.load(x + x_idx, mask=is_valid, other=0.0)
    k_value = tl.load(mix_k + feat_idx)
    
    # 处理第一个时间步
    is_first = seq_idx < 1
    prev_state_idx = batch_idx * hidden_dim + feat_idx
    prev_state = tl.load(x_prev + prev_state_idx, 
                        mask=(is_first & is_valid), 
                        other=0.0)
    
    # 加载前一个时间步的值
    prev_x_idx = x_idx - hidden_dim
    prev_x = tl.load(x + prev_x_idx,
                    mask=(~is_first & is_valid),
                    other=0.0)
    ...
    result = curr_x + mixed
    # 存储结果
    tl.store(output + x_idx, result, mask=is_valid)
...
def triton_rwkv_mix(x, x_prev, x_k):
    ...
    Args:
        x: 输入张量，形状可以是 (seq_len, hidden_dim) 或 (batch_size, seq_len, hidden_dim)
        x_prev: 前一状态，形状可以是 (hidden_dim) 或 (batch_size, hidden_dim)
        x_k: 混合系数，形状应为 (1, 1, hidden_dim)
        inplace: 是否进行原地操作
    """
    has_batch = x.dim() == 3
...
def relu_square_backward(
    ...
    # 加载数据
    x = tl.load(forward_input + offsets)
    grad = tl.load(grad_output + offsets)
    ...
    # 存储结果
    tl.store(grad_output + offsets, grad_input)
...
def rwkv_mix_backward(
    ...
    # 计算主要项
    dk1 = tl.load(dk1_ptr0 + xindex)
    xk = tl.load(xk_ptr + x0, eviction_policy='evict_last')
    prod = dk1 * xk
    
    mask = x1 < token_length-1
    prev_term = tl.where(mask,
                        tl.load(dk1_ptr0 + (n_embd + xindex), mask) *
                        tl.load(xk_ptr + x0, mask),
                        0.0)
    
    # 存储结果
    tl.store(dx_ptr + xindex, dk1 - prod + prev_term)
    tl.store(dx_prev_ptr + xindex, prod)



def rwkv_x070_channel_mixing_torch(x, x_prev, x_k, K_, V_):
    x_prev = x_prev.unsqueeze(1)  # (batch_size, 1, hidden_dim)
    xx = torch.cat((x_prev, x[:, :-1, :]), dim=1) - x
    k1 = x + xx * x_k
    k = torch.relu(k1 @ K_) ** 2
    return k @ V_, x[-1,:]
...
def rwkv_x070_channel_mixing_triton_backward(grad_output, x, x_prev, x_k, key_weight, value_weight, k1, k1_K, k):
    ...
    dx_prev = torch.zeros((batch_size, seq_len, n_embd), device=x.device, dtype=x.dtype)
...
class Rwkv7ChannelMixing(torch.autograd.Function):
    ...
    def backward(ctx, dkv):
        ...
        dx, dx_prev, dk_reduced, dK, dV = rwkv_x070_channel_mixing_triton_backward(dkv, x, x_prev, x_k, K_, V_, k1, k1_K, k)
        return dx, dx_prev, dk_reduced, dK, dV, None


def rwkv7_channel_mixing(x: torch.Tensor, x_prev: torch.Tensor, x_k: torch.Tensor, key_weight: torch.Tensor, value_weight: torch.Tensor, train_mode:bool = True):
    assert x.dim() == 3
